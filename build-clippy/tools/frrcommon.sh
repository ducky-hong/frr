...
# file does not exist.
#
# This script should be installed in  /usr/local/sbin/frrcommon.sh
...
D_PATH="/usr/local/sbin" # /usr/lib/frr
C_PATH="/usr/local/etc/frr${suffix}" # /etc/frr
V_PATH="/usr/local/var/run/frr${suffix}" # /var/run/frr
B_PATH="/usr/local/bin"
VTYSH="/usr/local/bin/vtysh" # /usr/bin/vtysh
FRR_USER="frr" # frr
FRR_GROUP="frr" # frr
...
DAEMONS="mgmtd zebra bfdd bgpd ripd ripngd ospfd ospf6d isisd babeld pimd pim6d ldpd nhrpd eigrpd sharpd pbrd staticd fabricd vrrpd pathd"
